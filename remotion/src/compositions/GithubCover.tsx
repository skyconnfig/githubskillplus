...
export const GithubCover: React.FC<GithubCoverProps> = ({ title, repoUrl, variant }) => {
  const portrait = variant === "3x4";
  const repoName = repoUrl.replace(/^https?:\/\/github\.com\//, "").replace(/\/$/, "");
  return (
    <AbsoluteFill style={{ background: "#061426", color: "#f8fbff", fontFamily: "Microsoft YaHei, Arial, sans-serif", overflow: "hidden", padding: portrait ? 76 : 68 }}>
      ...
      <div style={{ position: "absolute", width: portrait ? 560 : 760, height: portrait ? 560 : 440, right: portrait ? 60 : 54, bottom: portrait ? 64 : 92, borderRadius: 32, background: "linear-gradient(145deg,#f7fbff,#d8e5f3)", transform: portrait ? "rotate(3deg)" : "rotate(-4deg)", boxShadow: "0 30px 80px rgba(0,0,0,.45)" }}>
        ...
        <div style={{ padding: 36, color: "#10243b" }}>
          <div style={{ fontSize: portrait ? 30 : 36, fontWeight: 900 }}>{title}</div>
      ...
      <div style={{ position: "relative", zIndex: 1, width: portrait ? "100%" : "52%" }}>
        ...
        <div style={{ marginTop: portrait ? 42 : 52, fontSize: portrait ? 74 : 86, lineHeight: 1.05, fontWeight: 950, letterSpacing: -2, textShadow: "0 8px 0 rgba(0,0,0,.35)" }}>{title}</div>
        <div style={{ marginTop: 32, fontSize: portrait ? 30 : 36, lineHeight: 1.35, color: "#ffffff", fontWeight: 800 }}>自动分析 README，生成可运镜的项目视频</div>
      ...
      <div style={{ position: "absolute", left: portrait ? 76 : 68, bottom: portrait ? 42 : 42, color: "rgba(255,255,255,.62)", fontSize: 18 }}>IndexTTS · bbox framing · SVG hand-drawn motion</div>
    </AbsoluteFill>
  );
};
